...
const SectionTitle: React.FC<{ children: React.ReactNode, light?: boolean }> = ({ children, light }) => (
  <h2 className={`text-4xl md:text-5xl font-serif font-bold mb-8 ${light ? 'text-white' : 'text-stone-900 dark:text-stone-100'}`}>{children}</h2>
);
    ...
    <div className="pt-40 pb-40 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-24 items-start mb-40">
          <div className="text-stone-300 space-y-8 text-xl font-light">
            <p>{data.aboutText}</p>
            <p>We are a boutique sanctuary dedicated to the artistry of low-intervention viticulture. Every bottle in our shop is hand-selected, representing a commitment to soil health, transpar[...]</p>
          </div>
    ...
    <div className="pt-40 pb-40 px-4">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
        <div>
          ...
          <p className="text-stone-300 text-xl font-light leading-relaxed mb-16">Located in the heart of Hudson Yards, our shop is an industrial sanctuary for those who appreciate the finer, cleaner s[...]</p>
        ...
        <div className="h-[700px] border border-white/5 grayscale relative">
          <iframe 
            src={`https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3022.4497321289196!2d${shopLoc.lng}!3d${shopLoc.lat}!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x89c259ae65c9535b%3A0xc3...`}
      ...
      <div className={`w-72 bg-black border-r border-white/5 p-8 flex flex-col gap-2 fixed md:sticky h-screen top-0 transition-transform z-50 overflow-y-auto admin-scrollbar ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}> 
        ...
        {[
          ...
          <button 
            ...
            className={`flex items-center gap-4 p-4 rounded-sm text-xs font-bold uppercase tracking-widest transition-all ${activeTab === tab.id ? 'bg-custom-accent text-white' : 'text-stone-500 hover:text-white'}`} 
          >
        ...
        {activeTab === 'wines' && (
          <div>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-4">
              ...
              <button 
                onClick={() => setLocalData({ ...localData, wines: [{ id: Date.now().toString(), name: "New Bottle", region: "Region", price: 0, type: 'Red', description: "", imageUrl: "https://images.unsplash.com/photo-1510812431401-41d2bd2722f3?q=80&w=2070&auto=format&fit=crop" }, ...localData.wines] })
            ...
            <div className="flex justify-between items-center mb-12">
              ...
              <button 
                onClick={() => {
                  ...
                  setLocalData({ ...localData, posts: [{ id: Date.now().toString(), title: "New Entry", slug, excerpt: "Short summary...", content: "Full content here...", date: new Date().toISOString(), published: false, imageUrl: "" }, ...localData.posts] });
                }}
                className="bg-custom-accent px-6 py-3 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2"
              >
              ...
              {localData.posts.map((post, idx) => (
                <div key={post.id} className="bg-stone-900/50 p-8 border border-white/5">
                   ...
                   <div className="mt-8 pt-6 border-t border-white/5 flex justify-between">
                     <button onClick={() => setLocalData({ ...localData, posts: localData.posts.filter(p => p.id !== post.id) })} className="text-[10px] uppercase font-bold text-stone-500 hover:text-red-500 transition-colors flex items-center gap-2">
        ...
        {activeTab === 'services' && (
          <div>
            <div className="flex justify-between items-center mb-12">
              ...
              <button 
                onClick={() => setLocalData({ ...localData, services: [{ id: Date.now().toString(), title: "New Service", description: "Details...", imageUrl: "https://images.unsplash.com/photo-151081..." }, ...localData.services] })}
                className="bg-custom-accent px-6 py-3 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {localData.services.map((service, idx) => (
                <div key={service.id} className="bg-stone-900/50 p-8 border border-white/5">
                  ...
                  <div className="mt-8 pt-6 border-t border-white/5 flex justify-between">
                     <button onClick={() => setLocalData({ ...localData, services: localData.services.filter(s => s.id !== service.id) })} className="text-[10px] uppercase font-bold text-stone-500 hover:text-red-500 transition-colors flex items-center gap-2">
                       <Trash2 size={14} /> Remove
                     </button>
                  </div>
                </div>
                    ...
                    {localData.subscribers.map(sub => (
                      <tr key={sub.id} className="border-b border-white/5 hover:bg-white/5">
                        ...
                        <td className="p-6 text-right">
                          <button onClick={() => setLocalData({ ...localData, subscribers: localData.subscribers.filter(s => s.id !== sub.id) })} className="text-stone-600 hover:text-red-500 transition-colors">
                            <Trash2 size={14} />
                          </button>
                        </td>
                      </tr>
